...
from utils.util import flatten_lists
...
class Metrics(object):
    ...
    def __init__(self, golden_tags, predict_tags, remove_O=False):
        ...
        assert len(self.golden_tags) == len(self.predict_tags)
        ...
        print('golden_tags: ', self.golden_tags_counter)
        print('predict_tags: ', self.predict_tags_counter)
        print('correct_tags: ', self.correct_tags_number)
    ...
    def report_scores(self):
        ...
        total = 0
        for i in self.golden_tags_counter.keys():
            # if i == 'O':
            #     continue
            total += self.golden_tags_counter[i]

        # 计算并打印平均值
        avg_metrics = self._cal_weighted_average(total)
        print(row_format.format(
            'avg/total',
    ...
    def _cal_weighted_average(self, total):
